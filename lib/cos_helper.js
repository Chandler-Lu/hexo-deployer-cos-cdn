...
function loadLocalFiles(cfgs, publicDir, localFileMap, localImgsMap) {
  if (!cfgs.imageConfig) {
    ...
    getFiles(publicDir, (file) => {
      ...
      let strRegExp =
        '(src="|content="|href=")([^"]*?/' + cfgs.imageConfig.folder + '/)([^"]*?[.jpg|.jpeg|.png|.gif|.zip]")';
      let imgRegExp = new RegExp(strRegExp, "gi");
      //如果是图片目录，将目录内图片文件列表写入 localImgsMap 对象，将上传到单独的 bucket
    ...
    .then((uploadFiles) => {
      ...
      return uploadFile(cos, cfgs, uploadFiles)
        ...
        .then((filesMap) => {
          ...
          return cacheRefresh(cfgs, filesMap).then((res) => {
            if (res !== false) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (cfgs.cloud === "tencent") {
      ...
      filesMap.forEach((fileFullPath, filePath) => {
        ...
        urls.Urls.push(encodeURI(cfgs.cdnUrl + filePath));
        ++i;
      });
      ...
      QCSDK.request("PurgeUrlsCache", urls, (res) => {
        res = JSON.parse(res);
      ...
      aliyun.cdnRefresh(cfgs, urls).then(
        (result) => {
          resolve(result);
        },
        (err) => {
...
function putObject(cos, config, filePath, fileFullPath) {
  return new Promise((resolve, reject) => {
    ...
    if (config.cloud === "tencent") {
      ...
      oss
        .ossUpload(config, filePath, fileFullPath)
        .then(({ name, url }) => {
          resolve(url);
        })
        .catch((err) => {
...
function deleteFile(cos, config, fileList) {
  return new Promise((resolve, reject) => {
    ...
    if (config.cloud === "tencent") {
      ...
      oss
        .ossDeleteMulti(config, fileList)
        .then((deletedFiles) => {
...
function checkConfigs(config) {
  ...
  if (
    !cfgs.imageConfig.cdnUrl ||
    !cfgs.imageConfig.cloud ||
    !cfgs.imageConfig.bucket ||
    !cfgs.imageConfig.region ||
    !cfgs.imageConfig.folder ||
    !cfgs.imageConfig.secretId ||
    !cfgs.imageConfig.secretKey
  ) {
    let tips = [
  ...
  cfgs.imageConfig.deleteExtraFiles =
    cfgs.imageConfig.deleteExtraFiles === undefined ? false : cfgs.imageConfig.deleteExtraFiles;
  return cfgs;
}
